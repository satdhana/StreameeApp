...
function* getMoviesUpcoming(action) {
  try {
    const resMoviesUpcoming = yield axios({
      method: 'GET',
      url: `https://api.themoviedb.org/3/movie/top_rated?api_key=${API_KEY}&language=en-US&page=${action.page}`,
    });
    if (resMoviesUpcoming && resMoviesUpcoming.data) {
      yield put({
        type: 'GET_MOVIES_TOP_SUCCESS',
...
function* moviesSaga() {
  yield takeLatest('GET_MOVIES_TOP', getMoviesUpcoming);
}
